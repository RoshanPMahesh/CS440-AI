...
def bigramBayes(dev_set, train_set, train_labels, unigram_laplace=1.0, bigram_laplace=0.1, bigram_lambda=0.1, pos_prior=0.75, silently=False):
    ...
    for mixture_model in range(len(dev_set)):
        positives = ((1 - bigram_lambda) * uni_pos[mixture_model]) + ((bigram_lambda) * bi_pos[mixture_model])
        negatives = ((1 - bigram_lambda) * uni_neg[mixture_model]) + ((bigram_lambda) * bi_neg[mixture_model])
